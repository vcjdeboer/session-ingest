...
const bySource = (c: External, name: string) =>
  c.sources.find((s) => s.source === name);

Deno.test({
  name:
    "external: STRICT counts-only — NO call content leaks (args_json[1+]/data_inline/data_ref/creds)",
  ignore: !HAVE,
  fn: async () => {
    const org = await fixture();
    try {
      const { sink, resources } = fakeSink();
      const { external: c } = await captureExternal(
        org,
        "gdh",
        undefined,
        sink,
      ...
      for (const s of SECRETS) {
        assert(!blob.includes(s), `secret/content leaked: ${s}`);
      }
      // sanity: the safe server names ARE present (proves we captured, not just emptied)
      assert(
        blob.includes("pubmed") && blob.includes("opentargets"),
        "servers captured",
      );
    } finally {
...
Deno.test({
  name:
    "external: per-source aggregation + project scoping + byMethod histogram",
  ignore: !HAVE,
  fn: async () => {
    const org = await fixture();
    try {
      ...
      assert(
        !c.sources.some((s) => s.callCount === 1 && s.totalBytes === 999),
        "other-project call excluded",
      );
      // in-scope totals: mcp5 + credentials_request1 + get_user_email1 + llm1 = 8
      assertEquals(c.totals.calls, 8);
      assertEquals(c.totals.byMethod, {
        credentials_request: 1,
        get_user_email: 1,
        llm: 1,
        mcp: 5,
      });
      ...
      assertEquals(
        [...c.sources.map((s) => s.source)],
        [...c.sources.map((s) => s.source)].sort(),
      );
    } finally {
      await Deno.remove(org, { recursive: true });
    }
...
Deno.test({
  name:
    "external: malformed args_json does NOT abort (json_valid guard) → mcp:malformed bucket",
  ignore: !HAVE,
  fn: async () => {
    const org = await fixture();
    try {
      ...
      assert(
        c.warnings.some((w) => /mcp:malformed/.test(w)),
        "malformed row warned",
      );
    } finally {
      await Deno.remove(org, { recursive: true });
    }
...
Deno.test({
  name:
    "external: a structured args_json[0] → mcp:unrecognized, object text NOT leaked",
  ignore: !HAVE,
  fn: async () => {
    const org = await fixture();
    try {
      ...
      assert(
        !JSON.stringify(c).includes("SECRET-OBJ"),
        "structured $[0] content not leaked",
      );
      assert(
        c.warnings.some((w) => /mcp:unrecognized/.test(w)),
        "unrecognized row warned",
      );
    } finally {
      await Deno.remove(org, { recursive: true });
    }
...
Deno.test({
  name:
    "external: orphan (frame with no project) counted + warned, not silently dropped",
  ignore: !HAVE,
  fn: async () => {
    const org = await fixture();
    try {
      ...
      assert(
        c.warnings.some((w) => /unattributable to any project/.test(w)),
        "orphan warned",
      );
    } finally {
      await Deno.remove(org, { recursive: true });
...
Deno.test({
  name:
    "external: host_grants labeled filesystem mounts, host_path DROPPED (no abs path in record)",
  ignore: !HAVE,
  fn: async () => {
    const org = await fixture();
    try {
      ...
      assertEquals(c.filesystemMountGrants, [{
        mountName: "data",
        mode: "ro",
        userId: "u1",
      }]);
      assert(
        !JSON.stringify(c).includes("/Users/vincent/secret-mount"),
        "host_path not in the portable record",
      );
...
Deno.test({
  name:
    "external: secret-shaped args_json[0] → bucketed mcp:secret-shaped (never emitted) + valid-non-array → mcp:malformed",
  ignore: !HAVE,
  fn: async () => {
    ...
    try {
      ...
      assert(
        !JSON.stringify(c).includes("sk-abcdefghijklmnop"),
        "secret-shaped server never emitted as a key",
      );
...
Deno.test({
  name:
    "external: deterministic two-run byte-identical; source DB + no secret ciphertext leak",
  ignore: !HAVE,
  fn: async () => {
    const org = await fixture();
    ...
    try {
      ...
      assertEquals(
        JSON.stringify(r1.external),
        JSON.stringify(r2.external),
        "two runs byte-identical",
      );
      assert(
        !JSON.stringify(r1.external).includes("SUPERSECRETCIPHERTEXT"),
        "no user_secrets ciphertext",
      );
      assertEquals(
        await Deno.readFile(dbPath),
        dbBefore,
        "source DB byte-unmutated",
      );
    } finally {
      await Deno.remove(org, { recursive: true });
    }
