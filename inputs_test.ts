...
const MANIFEST =
  "run species common group\nSRR11884684 Mirounga elephant diver\nSRR9201556 Leptonychotes weddell diver\njunkline\n";
const ASSEMBLIES =
  '{"Mirounga":{"sci":"M. angustirostris","assembly":"GCF_029215605.1"}}';
...
async function makeOrg(): Promise<
  { csRoot: string; extBase: string; extReal: string }
> {
...
Deno.test({
  name:
    "inputs: copy + accession harvest + tripwire + over-cap by-reference + symlink skip",
  ignore: !HAVE,
  fn: async () => {
    ...
    try {
      ...
      const { inputs: c } = await captureInputs(
        csRoot,
        "gdh",
        undefined,
        sink,
      ...
      assert(
        root.referenced.some((x) => x.relPath === "big.bin"),
        "big.bin by-reference",
      );
      assert(
        !files[
          "blob/" +
          (root.copied.find((x) => x.relPath === "big.bin")?.sha ?? "")
        ],
        "big not blobbed",
      );
      ...
      assert(
        copied.includes("secret.env"),
        "secret still copied (sensitive bundle)",
      );
      // symlink escape NOT captured, and no absolute path leaked into the record
      assert(
        !copied.includes("link.txt") &&
          !root.referenced.some((x) => x.relPath === "link.txt"),
        "symlink skipped",
      );
      assert(
        !JSON.stringify(c).includes(outside),
        "no outside absolute path in the record",
      );
      // no absolute filesystem path (the ext base or the outside dir) leaks into warnings
      assert(
        !JSON.stringify(c.warnings).includes(extReal),
        "warnings carry no ext-base abs path",
      );
      assert(
        !JSON.stringify(c.warnings).includes(outside),
        "warnings carry no outside abs path",
      );
    } finally {
...
Deno.test({
  name:
    "inputs: referenceRoot records path+size only (no blob); accession still copied",
  ignore: !HAVE,
  fn: async () => {
    const { csRoot, extBase, extReal } = await makeOrg();
    ...
    try {
      ...
      const root = c.roots[0];
      assert(
        root.referenced.some((x) => x.relPath === "refbig.bin"),
        "refbig referenced",
      );
      ...
      assertEquals(
        ...
        root.copied.length,
        "only copied files blobbed",
      );
      // accession file still copied + parsed even inside a referenceRoot
      assert(
        root.copied.some((x) => x.relPath === "manifest.tsv"),
        "manifest copied in refRoot",
      );
      assert(
        c.accessions.sra.length === 2,
        "accessions harvested from refRoot manifest",
      );
    } finally {
      await Deno.remove(csRoot, { recursive: true });
      await Deno.remove(extBase, { recursive: true });
...
Deno.test({
  name:
    "inputs: most-specific-root-wins + canonical dedup across overlapping walk roots",
  ignore: !HAVE,
  fn: async () => {
    const { csRoot, extBase, extReal } = await makeOrg();
    ...
    try {
      ...
      const allCopied = c.roots.flatMap((r) =>
        r.copied.map((x) => `${r.resolvedRoot}|${x.relPath}`)
      );
      const fEntries = c.roots.flatMap((r) => r.copied).filter((x) =>
        x.relPath.endsWith("f.txt")
      );
      assertEquals(fEntries.length, 1, "f.txt deduped to one entry");
      assert(
        allCopied.some((s) => s.endsWith("/parent/sub|f.txt")),
        "owned by most-specific root",
      );
...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      assert(
        c.warnings.some((w) => /root rejected/.test(w)),
        "rejection warned",
      );
    } finally {
      await Deno.remove(csRoot, { recursive: true });
...
Deno.test({
  name:
    "inputs: a per-file IO failure is warned WITHOUT leaking an absolute path, and does not overcount totals (H1/M2)",
  ignore: !HAVE,
  fn: async () => {
    const { csRoot, extBase, extReal } = await makeOrg();
    ...
    try {
      ...
      assert(
        c.warnings.some((w) => /noperm\.txt/.test(w)),
      ...
      assert(
        !JSON.stringify(c.warnings).includes(extReal),
        "no abs path in any warning (H1)",
      );
      assert(
        !JSON.stringify(c).includes(`${extReal}/r/noperm.txt`),
        "the failing file's abs path never leaks",
      );
      // totals.files counts only files actually frozen, not the failed one (M2)
      const frozen = c.roots.reduce(
        (n, r) => n + r.copied.length + r.referenced.length,
        0,
      );
      assertEquals(
        c.totals.files,
        frozen,
        "totals.files == frozen count (no overcount)",
      );
    } finally {
...
Deno.test({
  name:
    "inputs: a referenceRoot outside the walk roots is TRULY ignored — files stay frozen, not silently by-referenced (H3)",
  ignore: !HAVE,
  fn: async () => {
    const { csRoot, extBase, extReal } = await makeOrg();
    ...
    try {
      ...
      assert(
        c.warnings.some((w) => /under no accepted walk root/.test(w)),
        "over-broad refRoot warned",
      );
      const root = c.roots[0];
      assert(
        root.copied.some((x) => x.relPath === "a.txt"),
        "a.txt COPIED (frozen), not silently by-referenced",
      );
      assert(
        !root.referenced.some((x) => x.relPath === "a.txt"),
        "a.txt NOT by-referenced",
      );
    } finally {
      await Deno.remove(csRoot, { recursive: true });
      await Deno.remove(extBase, { recursive: true });
...
Deno.test({
  name:
    "inputs: a non-manifest .tsv honors referenceOver instead of being force-copied (M1)",
  ignore: !HAVE,
  fn: async () => {
    const { csRoot, extBase, extReal } = await makeOrg();
    ...
    try {
      ...
      const root = c.roots[0];
      assert(
        root.referenced.some((x) => x.relPath === "abundance.tsv"),
        "big data .tsv by-reference (not hoarded)",
      );
      assert(
        !root.copied.some((x) => x.relPath === "abundance.tsv"),
        "big data .tsv NOT copied",
      );
      assert(
        root.copied.some((x) => x.relPath === "manifest.tsv"),
        "manifest.tsv still copied (recipe)",
      );
...
Deno.test({
  name:
    "inputs: a root listed twice yields ONE root entry, no double count (M5)",
  ignore: !HAVE,
  fn: async () => {
    const { csRoot, extBase, extReal } = await makeOrg();
    ...
    try {
      ...
      assertEquals(
        c.roots.reduce((n, r) => n + r.copied.length, 0),
        1,
        "file not double-listed",
      );
    } finally {
      await Deno.remove(csRoot, { recursive: true });
      await Deno.remove(extBase, { recursive: true });
...
Deno.test({
  name:
    "inputs: hashReferenced records a sha on referenced entries without writing a blob (M6)",
  ignore: !HAVE,
  fn: async () => {
    const { csRoot, extBase, extReal } = await makeOrg();
    ...
    try {
      ...
      assert(
        typeof e.sha === "string" && /^[0-9a-f]{64}$/.test(e.sha!),
        "referenced entry carries a sha",
      );
      assertEquals(
        Object.keys(files).length,
        0,
        "no blob bytes written for a referenced file",
      );
    } finally {
      await Deno.remove(csRoot, { recursive: true });
      await Deno.remove(extBase, { recursive: true });
...
Deno.test({
  name:
    "inputs: self-describing, NO drift field, deterministic two-run, source unmutated",
  ignore: !HAVE,
  fn: async () => {
    const { csRoot, extBase, extReal } = await makeOrg();
    ...
    const srcSha = async (p: string) =>
      [
        ...new Uint8Array(
          await crypto.subtle.digest("SHA-256", await Deno.readFile(p)),
        ),
      ]
    ...
    try {
      const a = fakeSink();
      const r1 = await captureInputs(csRoot, "gdh", undefined, a.sink, {
        roots: [`${extReal}/r`],
        allowedBases: [extReal],
      });
      const b = fakeSink();
      const r2 = await captureInputs(csRoot, "gdh", undefined, b.sink, {
        roots: [`${extReal}/r`],
        allowedBases: [extReal],
      });
      assertEquals(
        JSON.stringify(r1.inputs),
        JSON.stringify(r2.inputs),
        "two runs byte-identical",
      );
      const c: Inputs = r1.inputs;
      assert(c.capturedRoots.length === 1, "self-describing capturedRoots");
      assert(
        !JSON.stringify(c).includes('"drift"'),
        "no drift field (/tmp untracked)",
      );
      assertEquals(
        await srcSha(`${extBase}/r/a.txt`),
        before,
        "source file unmutated",
      );
    } finally {
      await Deno.remove(csRoot, { recursive: true });
      await Deno.remove(extBase, { recursive: true });
