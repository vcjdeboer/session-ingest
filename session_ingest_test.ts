...
Deno.test("session_ingest: identity + CalVer version", () => {
  ...
  assert(
    /^\d{4}\.\d{2}\.\d{2}\.\d+$/.test(m.version),
    `CalVer version, got ${m.version}`,
  );
});
...
Deno.test("session_ingest: all 8 read/capture/lock methods are wired", () => {
  for (
    ...
    assertEquals(
      typeof m.methods[name].execute,
      "function",
      `${name} has an execute fn`,
    );
  }
});
...
Deno.test("session_ingest: lock_env + offload file specs registered", () => {
  for (
    const f of ["body", "blob", "environment.yml", "dockerfile", "flake.nix"]
  ) {
